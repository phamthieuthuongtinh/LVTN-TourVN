                  ...
                  <tbody>
                    ...
                    <tr  class="{{ $com->status == 2 ? 'deleted-com' : '' }}">
                        ...
                        <td style="width: 500px;">{{$com->comment_content}}
                            ...
                            <li>
                                <div>
                                    <p style="display: inline; margin-right: 10px;">{{$comm_reply->comment_content}}</p>
                                    <form method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa?');" action="{{route('comment.destroy',[$comm_reply->comment_id])}}">
                                        @method('DELETE')
                                        @csrf
                                        <input style="font-size: 10px; display: inline;" type="submit" class="btn btn-danger" value="Xóa">
                                    </form>
                                </div>
                        ...
                        <td>
                            ...
                            <button  class="btn btn-info btn-sm btn-reply-comment" data-comment_id="{{$com->comment_id}}" data-tour_id="{{$com ->comment_tour_id}}" style="text-align:left;display: block; width: fit-content; margin-top: 5px;">Phản hồi</button>
                        </td>
                    </tr>
                    @endforeach
                  </tbody>
