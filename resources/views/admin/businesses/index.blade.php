                ...
                <table class="table table-striped table-valign-middle" id="myTable">
                  ...
                  <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($reg->status==1)
                            <a href="{{route('admin.accept_register',[$reg->id])}}"  class="btn btn-success">Duyệt</a>
                      
                                <br>
                                <form method="POST" onsubmit="return confirm('Bạn có chắc từ chối?');" action="{{route('admin.refuse_register',[$reg->id])}}">
                                    @method('PATCH')
                                    @csrf
                                <input type="submit" class="btn btn-danger" value="Từ chối">
                                </form>
                            @else
                            <form method="POST" onsubmit="return confirm('Bạn có chắc bỏ duyệt, các tải khoản được tạo cho doanh nghiệp sẽ bị xóa?');" action="{{route('admin.boduyet_register',[$reg->id])}}">
                                @method('PATCH')
                                @csrf
